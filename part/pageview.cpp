...
QString PageView::currentPageText() const
{
    if (!d->document || d->document->pages() == 0) {
        return {};
    }

    const Okular::Page *page = d->document->page(static_cast<int>(d->document->currentPage()));
    return page ? page->text(nullptr, Okular::TextPage::CentralPixelTextAreaInclusionBehaviour) : QString();
}
