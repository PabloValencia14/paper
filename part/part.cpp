    ...
    , m_keeper(nullptr)
{
    ...
    // Paper-only workspace: Markdown notes and a local assistant live in a
    // sidecar next to the document, so Okular's native PDF remains untouched.
    m_paperWorkspace = new PaperWorkspace(nullptr);
    m_paperWorkspace->setDocument(m_document, m_pageView);
    m_sidebar->addItem(m_paperWorkspace, QIcon::fromTheme(QStringLiteral("document-edit")), i18n("Paper"));
...
void Part::setupActions()
{
    ...
    QAction *paperWorkspaceAction = ac->addAction(QStringLiteral("paper_workspace"));
    paperWorkspaceAction->setText(i18n("Paper workspace"));
    paperWorkspaceAction->setIcon(QIcon::fromTheme(QStringLiteral("document-edit")));
    ac->setDefaultShortcut(paperWorkspaceAction, QKeySequence(Qt::CTRL | Qt::ALT | Qt::Key_P));
    connect(paperWorkspaceAction, &QAction::triggered, this, [this] {
        m_sidebar->setCurrentItem(m_paperWorkspace);
...
void Part::notifySetup(const QList<Okular::Page *> & /*pages*/, int setupFlags)
{
    ...
    m_paperWorkspace->setDocumentUrl(realUrl());
}
...
bool Part::closeUrl(bool promptToSave)
{
    ...
    if (m_paperWorkspace) {
        m_paperWorkspace->clearDocument();
    }
